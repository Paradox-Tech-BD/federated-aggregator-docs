...
export default function HospitalNode() {
  return (
    <div className="doc-page node-dossier">
      ...
      <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>five design artifacts</span></div>{documents.map((document, index) => <a key={document.file} className="node-document" href={`https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>)}</section>
      <section className="node-next"><div><span>NEXT IMPLEMENTATION GATE</span><h2>Build the SQLite adapter and fake Core ports before expanding the Core workload API.</h2></div><p>The next repository slice remains self-contained. Core changes begin only with separate workload authorization, audit, and contract tests—never by reusing a human administrator route.</p><CheckCircle2 size={24} /></section>
    </div>
  );
}
